...
export default {
  ...
  computed: {
    ...
    c_group_title_currentUser() {
      return `group_title_${this.currentUser.seq}`
    },
    c_group_favorite_address() {
      return `group_favorite_address_${this.currentUser.seq}`
    },
    c_condition_group_title: {
      get() {
        return filter(
          this.group_title_list,
      ...
      set(newlist) {
        ...
        newlist.forEach((list) => this.group_title_list.push(list))
      },
    },
    c_group_condition_info() {
      return filter(
        this.favorite_account_list,
        (list) =>
  ...
  watch: {
    selected_asset_seq() {
      ...
      if (this.active_sub_tab !== 'all') {
        this.active_sub_tab = 'all'
      }
    },
    active_sub_tab() {
      if (this.active_sub_tab !== 'all') {
  ...
  mounted() {
    if (localStorage.getItem(this.c_group_title_currentUser)) {
      this.group_title_list = JSON.parse(
        localStorage.getItem(this.c_group_title_currentUser)
      )
    }
    if (localStorage.getItem(this.c_group_favorite_address)) {
      this.favorite_account_list = JSON.parse(
        localStorage.getItem(this.c_group_favorite_address)
      )
    }
  },
  methods: {
    ...
    createGroup() {
      ...
      this.group_title_list.push({
        asset_seq: this.selected_asset_seq,
        name: this.group_title,
      })
      localStorage.setItem(
        this.c_group_title_currentUser,
        JSON.stringify(this.group_title_list)
      )
      this.group_title = ''
    },
    titleDelete() {
      if (this.delete_title !== null) {
        localStorage.removeItem(this.c_group_title_currentUser)
        localStorage.removeItem(this.c_group_favorite_address)
        this.group_title_list = filter(
          this.group_title_list,
          (list) => list.name !== this.delete_title
        )
        this.favorite_account_list = filter(
          this.favorite_account_list,
  ...
  <section :class="[$style.sectionWrap]">
            ...
            <PerfectScrollbar class="perfectScrollbar">
              ...
              >
                ...
                <div v-if="c_group_condition_info.length">
                  ...
                  >
                    <router-link :to="{ name: 'bank-account-balance' }">
    ...
    >
      <div :class="[$style.createGroupWrap]">
        ...
        <button @click="createGroup">그룹추가</button>
      </div>
    ...
    >
      <AddressRegistration
        ...
        :c_group_favorite_address="c_group_favorite_address"
        :c_condition_group_title="c_condition_group_title"
        :selected_asset_seq="selected_asset_seq"
        :group_title_list="group_title_list"
        :favorite_account_list="favorite_account_list"
      />
    </el-dialog>
  </section>
</template>
