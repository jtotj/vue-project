export default (function(_exports) {
  ...
  Scroll.prototype.keyStroke = function(hub, key, e) {
    ...
    if (key != 'up' && key != 'down' && key != 'enter' && key != 32)
      return false
...
<cq-advertisement style="display: block; height: 106px;">
  ...
  CIQ.UI.Advertisement = document.registerElement(
    'cq-advertisement',
    Advertisement
  )
      ...
      outer: for (var study in this.layout.studies) {
        ...
        if (self.messages.sources[type]) {
          for (var i = 0; i < this.markers.attribution.length; i++) {
            if (
              this.markers.attribution[i].params.panelName ==
              this.layout.studies[study].panel
            )
              continue outer
          }
  ...
  ChartTitle.prototype.setContext = function(context) {
    ...
    CIQ.UI.observe({
      ...
      action: 'callback',
      value: function() {
        if (self.context.stx.currentQuote())
          self.previousClose = self.context.stx.currentQuote().iqPrevClose
      },
    })
    this.initialize()
  }
  ...
  ChartTitle.prototype.update = function() {
    ...
    if (currentPrice && doUpdatePrice) {
      ...
      if (
        currentPriceDiv.textBetter(
          stx.formatYAxisPrice(currentPrice, stx.chart.panel)
        )
      ) {
        priceChanged = true
    ...
    if ((doUpdatePrice || doUpdateBrowserTab) && symbol && priceChanged) {
      ...
      var previousClose = this.previousClose
        ? this.previousClose
        : currentQuote
        ? currentQuote.iqPrevClose
        : null
      ...
      if (todaysChangeAbs) {
        todaysChangeDiv.textBetter(
          stx.formatYAxisPrice(todaysChangeAbs, stx.chart.panel)
        )
      }
...
<cq-comparison cq-marker>
  ...
  Comparison.prototype.renderLegend = function() {
    ...
    for (var s in stx.chart.series) {
      ...
      if (this.loading[series.parameters.symbolObject.symbol])
    ...
    if (q) {
      for (var s in stx.chart.series) {
        ...
        if (price.length) {
          ...
          if (newPrice && (newPrice.Close || newPrice.Close === 0))
            newPrice = newPrice.Close
  ...
  CIQ.UI.FibSettingsDialog = document.registerElement(
    'cq-fib-settings-dialog',
    FibSettingsDialog
  )
  ...
  CIQ.UI.LanguageDialog = document.registerElement(
    'cq-language-dialog',
    LanguageDialog
  )
...
<cq-lookup cq-keystroke-claim cq-keystroke-default>
  ...
  Lookup.prototype.keyStroke = function(hub, key, e, keystroke) {
    ...
    if (result) {
      ...
      if (
        ...
        (!input.value.length && (key == 'escape' || key == 'enter' || !focused))
      ) {
        this.close()
      } else {
        this.open()
  ...
  Undo.prototype.keyStroke = function(hub, key, e, keystroke) {
    if (key == 90 && (keystroke.ctrl || keystroke.cmd)) {
      // ctrl-z
      if (keystroke.shift) {
        this.redo()
      } else {
    ...
    if (key == 89 && (keystroke.ctrl || keystroke.cmd)) {
      // ctrl-y
      this.redo()
      return true
    }
   ...
   * @param {Number} [timeUnit] Optional chart timeUnit to use (leave empty for autodetect)
   * @alias set
   * @memberof WebComponents.cq-show-range
   * @since 5.1.1 timeUnit added
   */
  ShowRange.prototype.set = function(
    activator,
    multiplier,
    base,
    interval,
  ...
  CIQ.UI.StudyContext = document.registerElement(
    'cq-study-context',
    StudyContext
  )
...
<cq-themes>
  ...
  TimezoneDialog.prototype.open = function(params) {
    ...
    if (stx.displayZone) {
      ...
      currentUserTimeZone.text(
        stx.translateIf('Current TimeZone is') + ' ' + fullZone
      )
      button.show()
    } else {
      currentUserTimeZone.text(
  ...
  CIQ.UI.TimezoneDialog = document.registerElement(
    'cq-timezone-dialog',
    TimezoneDialog
  )
  ...
  DrawingToolbar.prototype.pickFillColor = function(activator) {
    ...
    if (!colorPickers.length) {
      console.log(
        'DrawingToolbar.prototype.getFillColor: no ColorPicker available'
      )
      return
    }
    var colorPicker = colorPickers[0]
  ...
  CIQ.UI.MenuDropDown = document.registerElement(
    'cq-menu-dropdown',
    MenuDropDown
  )
  ...
  ShareDialog.prototype.share = function() {
    ...
    CIQ.Share.createImage(stx, { hide: ['.stx_chart_controls'] }, function(
      data
    ) {
  ...
  CIQ.UI.AggregationDialog = document.registerElement(
...
<cq-menu class="ciq-menu ciq-studies collapse">
  ...
  StudyLegend.prototype.renderLegend = function() {
    ...
    if (
      CIQ.objLength(this.previousStudies) == CIQ.objLength(stx.layout.studies)
    ) {
    ...
    //will be in there.
    if (
      typeof markerLabel != 'undefined' &&
      this.node[0].childElementCount > 1
    ) {
      this.node.prepend(
  ...
  CIQ.UI.StudyParameter = document.registerElement(
    'cq-study-parameter',
    StudyParameter
  )
