export default (function(_exports) {
  ...
  var $ = jQuery
  var oo = require('./thirdparty/object-observe')
  var wc = require('./thirdparty/webcomponents-lite.min')
  var claims = []
   ...
   * @type {HTMLElement}
   */
  var BaseComponent = (CIQ.UI.BaseComponent = Object.create(
   ...
   * @memberof CIQ.UI
   */
  var DialogContentTag = (CIQ.UI.DialogContentTag = Object.create(
    BaseComponent
  ))
  ...
  CIQ.UI.StudyMenu = function(node, context, params) {
    ...
    this.alwaysDisplayDialog = this.params.alwaysDisplayDialog
      ? this.params.alwaysDisplayDialog
      : false
  ...
  CIQ.UI.StudyMenu.prototype.pickStudy = function(node, studyName) {
    ...
    if (
      handleSpecialCase(this.params.dialogBeforeAddingStudy, {
        stx: stx,
        name: studyName,
      })
    )
  ...
  CIQ.UI.HeadsUp.prototype.position = function() {
    ...
    function printValues() {
      ...
      if (prices) {
        if (prices.data) {
          ...
          if (visuals.length) {
            var relativeCandleSize = self.maxVolume.value
              ? prices.data.Volume / self.maxVolume.value
              : 0
        ...
        if (
          currentQuote &&
          currentQuote[plotField] &&
          self.tick == stx.chart.dataSet.length - 1
        ) {
          node
            .find('cq-hu-price')
            .text(stx.formatPrice(currentQuote[plotField]))
        }
      }
    }
    if (
      this.tick != this.prevTick ||
      (stx.chart.dataSegment && bar == stx.chart.dataSegment.length - 1)
    ) {
   ...
   * @param {string} aggregationType
   */
  CIQ.UI.Layout.prototype.showAggregationEdit = function(
    node,
    aggregationType
  ) {
  ...
  CIQ.UI.Layout.prototype.showPeriodicity = function(stx, params) {
    ...
    $(params.selector)
      .empty()
      .append(CIQ.translatableTextNode(stx, text))
  }
  ...
  CIQ.UI.Layout.prototype.getLanguage = function(node) {
    function showLanguage(params, node) {
      ...
      $(node)
        .find('cq-flag')
        .attr('cq-lang', CIQ.I18N.language)
    }
  ...
  UIManager.prototype.openMenu = function(menu, params) {
    ...
    setTimeout(function() {
      $(menu)
        .find('input[cq-focus]:first-child')
        .focus()
    }, 0)
