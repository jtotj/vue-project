...
;(function(definition) {
  ...
  if (typeof exports === 'object' && typeof module === 'object') {
    ...
    throw new Error(
      'Only CommonJS, RequireJS, and <script> tags supported for share.js.'
    )
  }
})(
  /* endRemoveIf(umd) */ function(_exports) {
    ...
    CIQ.Share.createImage = function(stx, params, cb) {
      ...
      if (widthPX && heightPX) {
        ...
        if (decorationObj) {
          renderedHeight =
            renderedHeight - decorationObj.headerPX - decorationObj.footerPX
        }
      } else if (heightPX) {
        if (decorationObj) {
          renderedHeight =
            heightPX - decorationObj.headerPX - decorationObj.footerPX
        }
        renderedWidth =
        ...
        getCanvas(
          ...
          function(err, canvas) {
            ...
            if (decorationObj) {
              ...
              decorationObj.decorate(
                stx,
                context,
                !imageType || imageType === 'full' ? null : true
              )
            }
    ...
    CIQ.Share.defaultDecorator.initialize = function(
      stx,
      canvas,
      widthPX,
      heightPX
    ...
    CIQ.Share.watermarkPanels = function(stx) {
      ...
      for (var p in stx.panels) {
        ...
        var w = stx.chart.context.measureText(
          panel.title.innerHTML.toUpperCase()
        ).width
        CIQ.semiRoundRect({
    ...
    CIQ.Share.brandMyChart = function(stx, imageURL, positioning, size) {
      function prependDisplayChart(stx, image, positioning, size) {
        return function() {
          ...
          stx.chart.context.drawImage(
            image,
            x,
            stx.panels.chart.top + y,
            width,
            height
          )
        }
      ...
      image.onload = (function(stx, prependDisplayChart, positioning) {
        return function() {
          CIQ.ChartEngine.prototype.prepend(
            'displayChart',
            prependDisplayChart(stx, this, positioning, size)
          )
          stx.draw()
        }
