export default (function(_exports) {
        ...
        for (var p in this.panels) {
          ...
          for (i = 0; i < ranges.length; i++) {
            ...
            chart.context.fillRect(
              ...
              thisPanel.bottom - thisPanel.top
            )
    ...
    CIQ.Marker.Tooltip.placementFunction = function(params) {
      ...
      for (var i = 0; i < params.arr.length; i++) {
        ...
        if (
          ...
          (stx.controls.crossY &&
            stx.controls.crossY.style.display == 'none') ||
          !(
      ...
      if (showSeries) {
        ...
        for (var renderer in renderers) {
          ...
          for (var id = 0; id < rendererToDisplay.seriesParams.length; id++) {
            ...
            var display =
              seriesParams.display || seriesParams.symbol || seriesParams.field
      ...
      if (supportedChartType) {
        ...
        var linearChart =
          (!this.mainSeriesRenderer ||
            !this.mainSeriesRenderer.standaloneBars) &&
  ...
  CIQ.RangeSlider = function(params) {
    ...
    var sliderContainer = $(
      '<div class="chartContainer" id="sliderContainer"></div>'
    )
    ...
    this.copyData = function(chart) {
      ...
      if (
        stx.quoteDriver &&
        stx.quoteDriver.behavior &&
        stx.quoteDriver.behavior.bufferSize
      )
    ...
    this.drawSlider = function() {
      ...
      var left = (self.tickLeft = Math.max(
        stx.tickFromPixel(chartSubholder.offsetLeft),
        0
      ))
      var right = (self.tickRight = Math.min(
        left + stx.chart.maxTicks - 2,
      ...
      ctx.fillRect(
        subholder.offsetLeft,
        subholder.offsetTop,
        pLeft - subholder.offsetLeft,
        subholder.offsetHeight
      )
      ctx.fillRect(
        subholder.offsetWidth,
        subholder.offsetTop,
        pRight - subholder.offsetWidth,
        subholder.offsetHeight
      )
    ...
    stx.prepend('resizeChart', function() {
      ...
      ciqChart.height(
        ciqChart.parent().height() -
          (ciqSlider.is(':visible') ? sliderHeight : 0)
      )
    ...
    $(subholder).on('mousemove touchmove pointermove', function(e) {
      ...
      var movement =
        touches && touches.length
          ? touches[0].pageX - e.target.offsetLeft
          : e.offsetX
