	...
	*/
	...
	CIQ.Marker.AboveCandle.placementFunction=function(params){
		...
		for(var i=0;i<params.arr.length;i++){
			...
			var bottom;
			if(useHighs){
				bottom=Math.round(height-stx.pixelFromPrice(quote.High, panel, yAxis)+bottomAdjust)+"px";
			}else{
				bottom=Math.round(height-stx.pixelFromPrice(quote.Close, panel, yAxis)+bottomAdjust)+"px";
			}
