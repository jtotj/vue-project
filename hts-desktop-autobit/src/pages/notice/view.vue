...
export default {
    ...
    computed: {
        c_document_srl() {
            return this.$route.params.document_srl
        }
    },
    methods: {
        init() {
            async.auto({
                document: (callback) => this.loadDocument(callback),
            })
        },
        ...
        loadDocument(callback) {
            this.$request((error, result) => {
                ...
                this.document = result.oDocument;

                callback(error);
            }, 'post', 'https://app.cocofx.com/notice', `act=dispBoardContentView&module=board&mid=notice&document_srl=${this.c_document_srl}`)
        }
    }
}
</script>
