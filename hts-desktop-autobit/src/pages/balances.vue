                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="coin in c_coin_list">
                                ...
                                <td>{{ coin.evaluated_estate === null ? $t('left_equal') : `${php.number_format(coin.evaluated_estate)} USDT` }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    <div v-for="tab in tabs" v-show="tab[0] === selected_tab" v-loading="request_loading && tab[0] != 'io_list'" :class="[{ on: tab[0] === selected_tab }, tab[0]]">
                        <template v-if="tab[0] === 'withdrawal'">
                            ...
                            <dl v-else class="input">
                                ...
                                <dd class="addr">
                                    <template v-if="c_pending_withdraw">{{ php.number_format(c_pending_withdraw.request_amount) }} USDT</template>
                                    <template v-else>
                        ...
                        <template v-else>
                            <div class="txt1">
                                <p class="deposit-notice">{{ $t('USDT_deposit_msg') }}</p>
                                <p class="deposit-notice">{{ $t('USDT_deposit_msg2') }}</p>
                                <p class="deposit-notice">{{ $t('USDT_deposit_msg3') }}</p>
                            </div>
                            <dl>
                                <dt>{{ $t('deposit_coin') }}</dt>
                                <dd style="color: #fff">{{ 'XRP' }}</dd>
                                <dt style="width: 55px;">{{ $t('wallet_address') }}</dt>
                                <dd style="width: calc(100% - 55px);">
                                    <v-popover>
                                        rsjXos2GgPECLkt3B5w2Yu14BTZd2SCEmA
                                        <template v-if="0" slot="popover">
                            ...
                            <dl class="input">
                               ...
                               <!-- <dd v-if="s_account_info">
                                ...
                                <dd v-if="c_pending_deposit" style="color: #fff;">
                                    {{ c_pending_deposit.asset_price ? (c_pending_deposit.request_amount / c_pending_deposit.asset_price).toFixed(8) : $t('not_calculation')}} {{ 'XRP' }}
                                    ({{ php.number_format(c_pending_deposit.request_amount) }} {{ 'USDT' }})
                                </dd>
                                <dd v-else class="addr">
                                    <vue-numeric v-restrict.number v-model.number="deposit_params.request_amount" type="text"></vue-numeric>
                                </dd>
                                <dt>{{$t('transaction')}}</dt>
                                <dd class="txid" style="color: #fff">
                            ...
                            <div class="warning">
                                ...
                                <ul v-if="s_account_info" class="io_btn">
                                    <template v-if="c_pending_deposit">
                                        ...
                                        <li v-if="0"><button type="button" @click.prevent="reApplyDeposit" class="deposit"><i class="fa fa-check" aria-hidden="true"></i>{{ $t('deposit_reapply') }}</button></li>
                                    </template>
                                    <template v-else>
...
export default {
    ...
    methods: {
        ...
        createRealDeposit() {
            return this.requestDemoDeposit();
