...
* For proper functionality, balances, openOrders, positions and trades must be loaded using their corresponding fetch methods and not set manually in the Account object.
*
...
/**
 * OpenOrders CIQ.Account.OpenOrders
...
CIQ.TFC.prototype.enableBracket = function(params) {
  ...
  if (params && params.trade && params.trade.protect) {
    if (params.trade.quantity > 0 && params.trade.protect.limit) {
      yAbove = this.stx.pixelFromPriceTransform(
    ...
    if (params.trade.quantity > 0 && params.trade.protect.stop) {
      yBelow = this.stx.pixelFromPriceTransform(
        params.trade.protect.stop,
        this.chart.panel
      )
  ...
  this.positionAboveLine(
    this.stx.valueFromPixelUntransform(yAbove, this.chart.panel)
  )
  this.positionBelowLine(
    this.stx.valueFromPixelUntransform(yBelow, this.chart.panel)
  )
  this.updateValues()
  this.render()
}
...
CIQ.TFC.prototype.enableStraddle = function() {
  ...
  var y = this.stx.pixelFromPriceTransform(this.centerPrice, this.chart.panel)
  this.positionAboveLine(
    this.stx.valueFromPixelUntransform(y - 50, this.chart.panel)
  )
  this.positionBelowLine(
    this.stx.valueFromPixelUntransform(y + 50, this.chart.panel)
  )
...
CIQ.TFC.prototype.updateValues = function() {
  ...
  if (this.activeTrade == 'market') {
    ...
    if (this.activeInput == 'shares') {
      ...
      if (amount > 0)
        CIQ.setValueIfNotActive(this.elements.marketCurrency, amount.toFixed(0))
    ...
    if (this.elements.dragLineCenterPrice.innerHTML !== '')
      price = this.quantityFromValue(
        this.elements.dragLineCenterPrice.innerHTML
      )
    if (this.activeInput == 'shares') {
  ...
  var abovePrice = this.quantityFromValue(
    this.elements.dragLineAbovePrice.innerHTML
  )
  var belowPrice = this.quantityFromValue(
    this.elements.dragLineBelowPrice.innerHTML
  )
  ...
  if (window.setMarketOrderPlaceholders)
    setMarketOrderPlaceholders(this, tradesLikeForex)
}
...
CIQ.TFC.prototype.priceFromLocation = function(y) {
  var price = this.stx.valueFromPixelUntransform(
...
CIQ.TFC.prototype.locationFromPrice = function(p) {
  return (
    this.stx.pixelFromPriceTransform(p, this.chart.panel) - this.chart.panel.top
  )
}
...
CIQ.TFC.prototype.deriveOpenOrderMarkers = function() {
  ...
  if (
    ...
    CIQ.hasClassName($$$('.stx-trade-panel'), 'closed')
  )
    return
...
CIQ.TFC.prototype.updateData = function() {
  ...
  function update() {
    ...
    $$$('.tfc-liquidity').innerHTML = CIQ.money(
      self.account.balances.liquidity,
      null,
      CIQ.convertCurrencyCode(self.account.currency)
    )
    $$$('.tfc-unsettled-cash').innerHTML = CIQ.money(
      self.account.balances.unsettledCash,
      null,
      CIQ.convertCurrencyCode(self.account.currency)
    )
    $$$('.tfc-cash').innerHTML = CIQ.money(
      self.account.balances.cash,
      null,
      CIQ.convertCurrencyCode(self.account.currency)
    )
    if (self.account.balances.profitLoss) {
      ...
      $$$('.tfc-profitloss').innerHTML = CIQ.money(
    ...
    $$$('.tfc-buying-power').innerHTML = CIQ.money(
      self.account.balances.buyingPower,
      null,
      CIQ.convertCurrencyCode(self.account.currency)
    )
      ...
      for (i = 0; i < openOrders.length; i++) {
        ...
        if (lastWasOco) {
          if (openOrder.action != openOrders[i - 1].action) {
            CIQ.newChild(tr, 'TD', null, openOrder.action.capitalize())
          } else {
            td = CIQ.newChild(tr, 'TD')
          }
          td = CIQ.newChild(tr, 'TD')
          ...
          //CIQ.safeClickTouch(tr, closure(symbol, self));
          //}
          CIQ.newChild(tr, 'TD', null, openOrder.action.capitalize())
    ...
    for (j = 0; j < symbolList.length; j++) {
      ...
      if (viewType == 'lots') {
        ...
        if (position.quantity > 0) {
          ...
          else if (position.basis > position.price)
            CIQ.appendClassName(td, 'down')
        } else {
          if (position.basis > position.price) CIQ.appendClassName(td, 'up')
          else if (position.basis < position.price)
            CIQ.appendClassName(td, 'down')
        ...
        //CIQ.newChild(tr, "TD", null, CIQ.money(position.price * position.quantity * contractSize, null, CIQ.convertCurrencyCode(position.currency));
        if (position.basis) {
          CIQ.newChild(
            tr,
            'TD',
          ...
          if (viewType == 'lots' || viewType == 'maintenance') {
            ...
            if (viewType == 'maintenance') {
              ...
              if (trade.quantity > 0) {
                ...
                else if (trade.basis > trade.price)
                  CIQ.appendClassName(td, 'down')
              } else {
                if (trade.basis > trade.price) CIQ.appendClassName(td, 'up')
                else if (trade.basis < trade.price)
...
CIQ.TFC.prototype.refreshScrollWindows = function(openOrderTop) {
  ...
  var toggle = $$$('.stx-trade-ticket-toggle.close')
  var totalHeight =
  ...
  if (openOrderTop) {
    ...
    if ($$$('.stx-trade-positions').style.display == 'none')
  ...
  if (this.positionIScroll === null) {
    this.positionIScroll = CIQ.iscroll.newScroller(positionWrapper, {
      tap: true,
      scrollbars: true,
  ...
  if (this.openOrderIScroll === null) {
    this.openOrderIScroll = CIQ.iscroll.newScroller(openOrderWrapper, {
      tap: true,
      scrollbars: true,
      interactiveScrollbars: true,
    })
...
CIQ.TFC.prototype.closeTFC = function() {
  ...
  this.clearActive()
  this.hideAllDOM()
  if (CIQ.hasClassName($$$('.stx-trade-panel'), 'closed')) {
...
CIQ.TFC.prototype.cancelOpenOrder = function() {
  ...
  CIQ.unappendClassName($$$('#tfcConfirmOrder'), 'tfc-pending')
  ...
  this.account.confirmOrder(this, order, function(confirmation) {
    CIQ.DialogManager.displayDialog('tfcConfirmOrder')
    ...
    if (order.oto) {
      ...
      $$$('#tfcConfirmOrder .tfcOrderOTO').innerHTML = self.printableOTO(
        order.oto
      )
    } else {
    ...
    function closure(self, order, submitButton) {
      return function() {
        ...
        $$$('#tfcConfirmOrder .processOrder').style.display = 'block'
        CIQ.appendClassName($$$('#tfcConfirmOrder'), 'tfc-pending')
        self.account.cancelOrder(self, order, function(err) {
          CIQ.DialogManager.dismissDialog()
          processCancel(self, order, err)
        })
    ...
    CIQ.safeClickTouch(abandonButton, CIQ.DialogManager.dismissDialog)
    var i, div
    CIQ.clearNode($$$('#tfcConfirmOrder .tfc-errors'))
    if (confirmation && confirmation.errors) {
      submitButton.style.display = 'none'
      for (i = 0; i < confirmation.errors.length; i++) {
        div = CIQ.newChild(
          $$$('#tfcConfirmOrder .tfc-errors'),
          'div',
          null,
    ...
    if (confirmation && confirmation.warnings) {
      ...
      CIQ.clearNode($$$('#tfcConfirmOrder .tfc-warnings'))
    }
...
CIQ.TFC.prototype.createOrderFromGUI = function(action) {
  ...
  if (
    ...
    if (
      (action == 'market_buy' &&
        CIQ.hasClassName(this.elements.marketBuy, 'disabled')) ||
      (action == 'market_sell' &&
        CIQ.hasClassName(this.elements.marketSell, 'disabled'))
    ) {
      order.type = ''
      return
    }
...
CIQ.TFC.prototype.confirmReplace = function(order) {
  ...
  function closure(self, order, submitButton) {
    return function() {
      CIQ.clearSafeClickTouches(submitButton)
      CIQ.clearSafeClickTouches(abandonButton)
      CIQ.appendClassName($$$('#tfcConfirmReplace'), 'tfc-pending')
  ...
  this.modalEnd()
  CIQ.DialogManager.displayDialog('tfcConfirmReplace')
  CIQ.unappendClassName($$$('#tfcConfirmReplace'), 'tfc-pending')
    ...
    function closure(self, order, submitButton) {
      return function() {
        if (!order.quantity && order.quantity !== 0)
          order.quantity = $$('vsp_quantity').value
...
CIQ.TFC.prototype.confirmCloseAllPositions = function() {
  ...
  function closure() {
    ...
    CIQ.clearSafeClickTouches(abandonButton)
    $$$('#tfcConfirmCloseAll .processOrder').style.display = 'block'
    CIQ.appendClassName($$$('#tfcConfirmCloseAll'), 'tfc-pending')
...
CIQ.TFC.prototype.confirmOCO = function(order, type) {
  ...
  function closure(self, order, submitButton) {
    return function() {
      ...
      CIQ.appendClassName($$$('#tfcConfirmOCO'), 'tfc-pending')
      self.placeOrder(order)
    }
  }
  CIQ.safeClickTouch(submitButton, closure(this, order, submitButton))
  ...
  this.modalEnd()
  CIQ.DialogManager.displayDialog('tfcConfirmOCO')
  CIQ.unappendClassName($$$('#tfcConfirmOCO'), 'tfc-pending')
 ...
 */
CIQ.TFC.prototype.adjustMarker = function() {
...
CIQ.TFC.prototype.setResizeCallback = function(resizeCallback) {
  var self = this
  CIQ.safeClickTouch(
    $$$('.stx-trade-nav .stx-trade-ticket-toggle'),
    (function(stx) {
      return function() {
        stx.preAdjustScroll()
...
CIQ.TFC.prototype.changePositionsView = function(viewType) {
  ...
  for (var node = 0; node < nodes.length; node++) {
    if (CIQ.hasClassName(nodes[node], viewType))
      CIQ.appendClassName(nodes[node], 'active')
...
CIQ.TFC.prototype.construct = function(config) {
  ...
  this.elements.limitRiskReward = $$$(
    '.risk-reward .stx-data',
    this.dom.limitOrder
  )
  this.elements.ocoRiskReward = $$$(
  ...
  this.elements.cancelDescription = $$$(
  ...
  this.elements.useDollarsLabel = document.querySelectorAll(
    '.stx-label.tfc-price-dollars'
  )
  this.elements.useAmountLabel = document.querySelectorAll(
    '.stx-label.tfc-price-amt'
  )
  this.elements.useSharesLabel = document.querySelectorAll(
    '.stx-label.tfc-shares-to-buy'
  )
  this.elements.useUnitsLabel = document.querySelectorAll(
    '.stx-label.tfc-qty-to-buy'
  )
  this.elements.usePipsLabel = document.querySelectorAll('.stx-label.tfc-pips')
  this.elements.usePointsLabel = document.querySelectorAll(
    '.stx-label.tfc-points'
  )

  this.templates.openOrderMarker = $$$('.tfc .open-order-marker', container)

  this.menu.enableMarket.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-market'
  )
  this.menu.enableBuy.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-buy'
  )
  this.menu.enableSell.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-sell'
  )
  this.menu.enableShort.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-short'
  )
  this.menu.enableCover.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-cover'
  )
  this.menu.enableStraddle.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-straddle'
  )
  this.menu.enableStrangle.nodes = document.querySelectorAll(
    '.stx-trade-panel .stx-strangle'
  )
  this.menu.enableBracket.nodes = document.querySelectorAll(
