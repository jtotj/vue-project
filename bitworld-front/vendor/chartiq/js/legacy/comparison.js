;(function(definition) {
  ...
  if (typeof exports === 'object' && typeof module === 'object') {
    ...
    throw new Error(
      'Only CommonJS, RequireJS, and <script> tags supported for comparison.js.'
    )
  }
})(function(_exports) {
  ...
  /**
   * ** This function is maintained for legacy implementations only (not using web components). New implementations should use functionality included in the web components **<br>
  ...
  CIQ.Comparison.add = function(
    ...
    if (!compareSymbol)
      compareSymbol = $$$('#compareSymbol').value.toUpperCase()
    if (!compareSymbol) {
      if (cb) cb()
      return
    }
    ...
    function processResponse(symbol) {
      return function(err) {
        ...
        if (!stx.comparisons[symbol]) {
          ...
          if (stx.chart.series[symbol].parameters.permanent)
            $$$('.stx-ico-close', div).style.display = 'none'
          else {
            $$$('.stx-ico-close', div).onclick = (function(stx, symbol) {
              return function() {
                stx
                  ...
                  .removeSeries(symbol)
                  .ready()
                //CIQ.Comparison.removeSeries(stx,symbol); done by eventListenrer to also handle right click
              }
            })(stx, symbol)
          }
          div.style.display = ''
    ...
    CIQ.Comparison.fetch(
      stx,
      compareSymbol,
      parameters,
      processResponse(compareSymbol)
    )
  }
  ...
  CIQ.Comparison.quoteFeedCallback = function(params) {
    ...
    function getStartDate(symbol) {
      for (var c = params.stx.masterData.length - 1; c >= 0; c--) {
        if (
          params.stx.masterData[c] &&
          typeof params.stx.masterData[c][symbol] != 'undefined'
        ) {
    ...
    if (CIQ.Studies) {
      for (var s in CIQ.Studies.studySymbols.symbols) {
        syms[s] = true
      }
    }
    ...
    for (field in syms) {
      ...
      if (seriesParam.update) {
        ...
        if (!seriesParam.startDate)
          seriesParam.startDate = params.stx.masterData[0].DT
        if (!seriesParam.endDate)
          seriesParam.endDate =
            params.stx.masterData[params.stx.masterData.length - 1].DT
      }
  ...
  CIQ.Comparison.correlate = function(stx, symbol) {
    ...
    var correlationPeriod = parseInt(
      $$$('.stxCorrelate .stx-input-field').value,
      10
    )

    var corrPanel =
      stx.panels[
        CIQ.Comparison.correlationPanel + ' (' + correlationPeriod + ')'
      ]
    var inputs = {
    ...
    if (corrPanel) {
      for (
        var i = 0;
        i < stx.layout.studies[corrPanel.name].inputs['Compare To'].length;
        i++
      ) {
        inputs['Compare To'].push(
          stx.layout.studies[corrPanel.name].inputs['Compare To'][i]
        )
      }
    ...
    for (var panel in stx.panels) {
      if (
        stx.panels[panel].name.indexOf(CIQ.Comparison.correlationPanel) === 0
      ) {
        var compareArray =
          stx.layout.studies[stx.panels[panel].name].inputs['Compare To']
        for (var ii = 0; ii < compareArray.length; ii++) {
          if (compareArray[ii] == symbol) {
            stx.layout.studies[stx.panels[panel].name].outputs[
              'Result ' + symbol
            ] = CIQ.Comparison.colorSelection
          }
        }
  ...
  CIQ.Comparison.toggleCorrelate = function(stx) {
    ...
    if (display) {
      CIQ.unappendClassName(
        display,
        (!CIQ.Comparison.requestCorrelation).toString()
      )
