...
;(function(definition) {
  ...
  if (typeof exports === 'object' && typeof module === 'object') {
    ...
    throw new Error(
      'Only CommonJS, RequireJS, and <script> tags supported for ui.js.'
    )
  }
  ...
  CIQ.horizontalIntersect = function(vector, x, y) {
    if (
      x < Math.max(vector.x0, vector.x1) &&
      x > Math.min(vector.x0, vector.x1)
    )
      return true
    return false
  }
  ...
  CIQ.boxedIntersect = function(vector, x, y) {
    if (
      ...
      return false
    if (
      y > Math.max(vector.y0, vector.y1) ||
      y < Math.min(vector.y0, vector.y1)
    )
      return false
    return true
  }
  ...
  /**
   * ** This namespace is maintained for legacy implementations only (not using web components).
   * New implementations should use {@link CIQ.NameValueStore} or functionality included in the web components (componentUI.js) **<br>
   *
  ...
  /**
   * ** This namespace is maintained for legacy implementations only (not using web components). New implementations should use functionality included in the web components (componentUI.js) **<br>
   *
  ...
  CIQ.DialogManager.displayDialog = function(id) {
    ...
    if (CIQ.DialogManager.useOverlay && !CIQ.DialogManager.bodyOverlay) {
      CIQ.DialogManager.bodyOverlay = CIQ.newChild(
        document.body,
        'DIV',
        'stxDialogOverlay'
      )
  ...
  CIQ.MenuManager.destroy = function() {
    ...
    if (CIQ.MenuManager.bodyOverlay) {
      CIQ.MenuManager.bodyOverlay.parentNode.removeChild(
  ...
  CIQ.MenuManager.registerChart = function(stx) {
    ...
    if (!CIQ.MenuManager.bodyOverlay) {
      CIQ.MenuManager.bodyOverlay = CIQ.newChild(
        document.body,
        'DIV',
        'stxBodyOverlay'
      )
  ...
  CIQ.MenuManager.menuOn = function(name, callback, cascading) {
    ...
    CIQ.MenuManager.stack.push({
      closeCurrentMenu: callback,
      cascading: cascading,
    })
  ...
  CIQ.MenuManager.makeMenus = function() {
    function toggle(div, menu) {
      return function(e) {
        ...
        do {
          if (dom.className && dom.className.indexOf('menuOutline') != -1)
            return // clicked inside the menuDisplay and not the menu button
        ...
        if (div.style.display == 'none') {
          ...
          if (
            (CIQ.MenuManager.menusDisabled && !menu.alwaysOn) ||
            CIQ.MenuManager.menusDisabledDialog
          )
            return
          ...
          if (div.className.indexOf('menuScroll') != -1) {
            if (!this.iscroll) {
              this.iscroll = CIQ.iscroll.newScroller(div, {
                tap: true,
                scrollbars: false,
    ...
    function activate(menuOutline) {
      ...
      try {
        ...
        eval(action) /*jshint +W061 */ /*eslint-enable no-eval */
        /*jslint evil: false */
      }
    }
    ...
    for (var i = 0; i < CIQ.MenuManager.menus.length; i++) {
      ...
      for (var j = 0; j < clickables.length; j++) {
        CIQ.safeClickTouch(
          clickables[j],
          menuSafeClick(menuOutline, clickables[j])
        )
      }
    }
  }
  ...
  CIQ.MenuManager.disableMenus = function() {
    ...
    for (var i = 0; i < CIQ.MenuManager.menus.length; i++) {
      var menu = CIQ.MenuManager.menus[i]
      if (CIQ.MenuManager.onClass)
        CIQ.unappendClassName(menu, CIQ.MenuManager.onClass)
      if (CIQ.MenuManager.offClass)
        CIQ.appendClassName(menu, CIQ.MenuManager.offClass)
    }
  }
  ...
  CIQ.MenuManager.enableMenus = function() {
    ...
    for (var i = 0; i < CIQ.MenuManager.menus.length; i++) {
      var menu = CIQ.MenuManager.menus[i]
      if (CIQ.MenuManager.offClass)
        CIQ.unappendClassName(menu, CIQ.MenuManager.offClass)
      if (CIQ.MenuManager.onClass)
        CIQ.appendClassName(menu, CIQ.MenuManager.onClass)
    }
  }
  ...
  CIQ.MenuManager.closeThisMenu = function(el) {
    while (
      ...
      el.className.indexOf('menuOutline') == -1
    ) {
      el = el.parentNode
    }
    if (el && el.style) {
   ...
   * @memberOf CIQ.MenuManager
   */
  CIQ.MenuManager.attachColorPicker = function(
    colorClick,
    cpHolder,
    cb,
    noMenuBehavior
    ...
    colorClick[CIQTouchAction] = (function(fc, cpHolder) {
      return function() {
        if (!noMenuBehavior)
          CIQ.MenuManager.menuOn('colorPicker', closeMe(cpHolder))
        if (!cpHolder.colorPickerDiv) {
        ...
        if (x + cpHolder.colorPickerDiv.offsetWidth > CIQ.pageWidth())
          x = CIQ.pageWidth() - cpHolder.colorPickerDiv.offsetWidth - 20
        cpHolder.colorPickerDiv.style.left = x + 'px'

        var y = xy.y
        if (y + cpHolder.colorPickerDiv.clientHeight > CIQ.pageHeight())
          y = CIQ.pageHeight() - cpHolder.colorPickerDiv.offsetHeight - 20
  ...
  /**
   * ** This namespage is maintained for legacy implementations only (not using web components). New implementations should use functionality included in the web components (componentUI.js) **<br>
   ...
   * @param {CIQ.ChartEngine} stx - a chart
  ...
  CIQ.ThemeManager.populateDialog = function(id, stx) {
    ...
    function toggleBorders(noDraw) {
      if ($$$('#candleBordersOn', $$(id)).checked) {
        stx.styles.stx_candle_up['border-left-color'] = $$(id).querySelectorAll(
          '.stx-border-color.stx_candle_up'
        )[0].style.backgroundColor
        stx.styles.stx_candle_down['border-left-color'] = $$(
          id
        ).querySelectorAll(
          '.stx-border-color.stx_candle_down'
        )[0].style.backgroundColor
      } else {
    ...
    function toggleMountainGradient() {
      if (mountainGradientCheckbox && mountainGradientCheckbox.checked) {
        stx.styles.stx_mountain_chart.color = CIQ.hexToRgba(
          $$(id).querySelectorAll('.stx-border-color.stx_mountain')[0].style
            .backgroundColor,
          1
        )
        stx.styles.stx_mountain_chart.backgroundColor = CIQ.hexToRgba(
          $$(id).querySelectorAll('.stx-border-color.stx_mountain')[0].style
            .backgroundColor,
          50
        )
      } else {
        stx.styles.stx_mountain_chart.color = $$(id).querySelectorAll(
          '.stx-border-color.stx_mountain'
        )[0].style.backgroundColor
        stx.styles.stx_mountain_chart.backgroundColor = $$(id).querySelectorAll(
          '.stx-border-color.stx_mountain'
        )[0].style.backgroundColor
      }
      ...
      return function(color) {
        ...
        if (mapping) {
          for (var mapped in mapping) {
            ...
            if (className == 'stx_mountain') {
              // Hacked in here. Ideally we would expand class mapping to accept specific css fields
              if (
                mountainGradientCheckbox &&
                mountainGradientCheckbox.checked
              ) {
                stx.styles[mapped].color = CIQ.hexToRgba('#' + color, 1)
                stx.styles[mapped].backgroundColor = CIQ.hexToRgba(
                  '#' + color,
                  50
                )
              } else {
    ...
    for (var className in CIQ.ThemeManager.classMapping) {
      ...
      if (mapping) {
        ...
        if (CIQ.isTransparent(color) && className == 'backgroundColor')
          color = stx.containerColor
      }
      ...
      if (picker) {
        ...
        if (!picker[CIQTouchAction]) {
          CIQ.MenuManager.attachColorPicker(
            picker,
            CIQ.DialogManager,
            chooseColor('border-left-color', className)
          )
        }
        if (borderColor && borderColor != 'transparent')
          $$$('#candleBordersOn', $$(id)).checked = true
        if (mountainGradientCheckbox && color && color.indexOf('rgba(' === 0))
          mountainGradientCheckbox.checked = true
      }
    }
    toggleBorders(true)
  }
    ...
    for (var className in CIQ.ThemeManager.classMapping) {
      ...
      if (mapping) {
        ...
        if (style.borderTopColor)
          theme[className].borderTopColor = style.borderTopColor
        if (style.backgroundColor)
          theme[className].backgroundColor = style.backgroundColor
        if (
  ...
  CIQ.ThemeManager.saveTheme = function(name, stx) {
    ...
    if (CIQ.ThemeManager.storageCB)
      CIQ.ThemeManager.storageCB(JSON.stringify(CIQ.ThemeManager.themes), stx)
    CIQ.ThemeManager.themesToMenu(
      CIQ.ThemeManager.el,
      CIQ.ThemeManager.el2,
      CIQ.ThemeManager.stx,
      CIQ.ThemeManager.storageCB
    )
  }
  ...
  CIQ.ThemeManager.deleteTheme = function(stx, theme) {
    if (CIQ.ThemeManager.themes.customThemes[theme]) {
      if (theme == CIQ.ThemeManager.themes.enabledTheme)
        CIQ.ThemeManager.enableBuiltInTheme(stx, CIQ.ThemeManager.baseTheme)
  ...
  CIQ.ThemeManager.setThemes = function(obj, stx) {
    if (obj) {
      if (obj.customThemes)
        CIQ.ThemeManager.themes.customThemes = obj.customThemes
        ...
        if (mapping) {
          for (var mapped in mapping) {
            ...
            if (obj[className]['border-left-color']) {
              stx.styles[mapped]['border-left-color'] =
                obj[className]['border-left-color']
            }
            if (className == 'stx_mountain') {
              // Hacked in. See other note.
              stx.styles[mapped].backgroundColor =
                obj[className].backgroundColor
              stx.styles[mapped].borderTopColor = obj[className].borderTopColor
              if (!stx.styles[mapped].backgroundColor)
                stx.styles[mapped].backgroundColor = obj[className].color
              if (!stx.styles[mapped].borderTopColor)
                stx.styles[mapped].borderTopColor = obj[className].color
            }
          }
        } else {
          if (stx.chart.container)
            stx.chart.container.style[className] = obj[className]
        }
      }
      if (stx.chart.container) {
   ...
   * @param {CIQ.ChartEngine} stx The chart object
   * @param {string} theme The theme to load. Pass null to remove the current built in theme (not supported in multi chart mode).
  ...
  CIQ.ThemeManager.loadBuiltInTheme = function(stx, theme, cb) {
    ...
    for (var thisTheme in CIQ.ThemeManager.builtInThemes) {
      ...
      if (stx.wrapper) CIQ.unappendClassName($$$(stx.wrapper), themeName)
      else CIQ.unappendClassName($$$('body'), themeName)
    }

    themeName = CIQ.ThemeManager.builtInThemes[theme]
    if (typeof themeName != 'string') themeName = theme

    if (stx.wrapper) CIQ.appendClassName($$$(stx.wrapper), themeName)
    else CIQ.appendClassName($$$('body'), themeName)
  ...
  CIQ.ThemeManager.themesToMenu = function(el, el2, stx, cb) {
    ...
    for (var i = 0; i < els.length; i++) {
      if (els[i].style.display == 'block') el.removeChild(els[i])
    }
  ...
  /**
   * ** This class is maintained for legacy implementations only (not using web components). New implementations should use functionality included in the web components (componentUI.js) **<br>
  ...
  CIQ.DrawingToolbar.prototype.initialize = function(htmlElement) {
    ...
    function setLineColor(self) {
      return function(color) {
        if (color == '000000' || color == 'ffffff')
          self.stx.currentVectorParameters.currentColor = 'auto'
    ...
    if (this.stx.currentVectorParameters.currentColor == 'auto')
      this.stx.currentVectorParameters.currentColor =
        lineColorPicker.style.backgroundColor
    else
    ...
    if (this.stx.currentVectorParameters.fillColor == 'auto')
      this.stx.currentVectorParameters.fillColor =
        fillColorPicker.style.backgroundColor
    else
    ...
    if (display) {
      CIQ.unappendClassName(
        display,
        !this.stx.currentVectorParameters.axisLabel
      )
    ...
    if (this.stx.currentVectorParameters.annotation.font.style == 'italic')
      CIQ.appendClassName(
        $$$('.stx-toolbar .stx-annotation-italic', htmlElement),
        'active'
      )
    if (this.stx.currentVectorParameters.annotation.font.weight == 'bold')
      CIQ.appendClassName(
        $$$('.stx-toolbar .stx-annotation-bold', htmlElement),
        'active'
      )
    if (this.stx.currentVectorParameters.annotation.font.size)
      CIQ.DrawingToolbar.setFont(
        htmlElement,
        this.stx.currentVectorParameters.annotation.font.size
      )
    if (this.stx.currentVectorParameters.annotation.font.family)
      CIQ.DrawingToolbar.setFont(
        htmlElement,
        this.stx.currentVectorParameters.annotation.font.family
      )
    this.setLine(
      this.stx.currentVectorParameters.lineWidth,
      this.stx.currentVectorParameters.pattern
    )
  }
  ...
  CIQ.DrawingToolbar.prototype.setLine = function(width, pattern) {
    ...
    if (
      this.stx.currentVectorParameters.lineWidth ==
      Math.floor(this.stx.currentVectorParameters.lineWidth)
    )
  ...
  CIQ.DrawingToolbar.prototype.toggleAxisLabel = function() {
    this.stx.currentVectorParameters.axisLabel = !this.stx
      .currentVectorParameters.axisLabel
    var display = $$$('.stx-toolbar .stxAxisLabel', this.htmlElement)
    if (display) {
      CIQ.unappendClassName(
        display,
        (!this.stx.currentVectorParameters.axisLabel).toString()
      )
      CIQ.appendClassName(
        display,
        this.stx.currentVectorParameters.axisLabel.toString()
  ...
  CIQ.DrawingToolbar.setFont = function(div, txt) {
    ...
    if (txt == 'italic') {
      ...
      $$$(
        '.stx-toolbar .stx-annotation-size > span',
        self.htmlElement
      ).innerHTML = CIQ.stripPX(txt)
    } else if (txt == 'Default') {
      self.stx.currentVectorParameters.annotation.font.family = null
      $$$(
        '.stx-toolbar .stx-annotation-family > span',
        self.htmlElement
      ).innerHTML = txt
    } else {
  ...
  CIQ.DrawingToolbar.prototype.setVectorType = function(vectorType) {
    ...
    if (!vectorType) {
      ...
      $$$('#toolSelection', this.htmlElement).appendChild(
        CIQ.translatableTextNode(stx, 'Select Tool')
      )
      return
    }
    for (i in CIQ.DrawingToolbar.configurator) {
    ...
    if (
      stx.currentVectorParameters.pattern == 'none' &&
      !CIQ.DrawingToolbar.configurator['.stxToolbarNone'][vectorType]
    )
      this.setLine(stx.currentVectorParameters.lineWidth, 'solid')
    stx.changeVectorType(vectorType)
    $$$('#toolSelection', this.htmlElement).innerHTML = ''
    $$$('#toolSelection', this.htmlElement).appendChild(
      CIQ.translatableTextNode(stx, vectorType.capitalize())
    )
  ...
  CIQ.DrawingToolbar.setVectorType = function(stx, vectorType, div) {
    var self = CIQ.DrawingToolbar.findInstance(div)
    if (!self) {
      // First time through we initialize the default instance
      self = CIQ.DrawingToolbar._default = new CIQ.DrawingToolbar(
        $$$('.stx-toolbar'),
        stx
      )
    }
  ...
  CIQ.DrawingToolbar.prototype.crosshairs = function(state) {
    ...
    if (state) {
      $$$('#toolSelection', this.htmlElement).appendChild(
        CIQ.translatableTextNode(stx, 'Crosshairs')
      )
    } else {
      $$$('#toolSelection', this.htmlElement).appendChild(
        CIQ.translatableTextNode(stx, 'Select Tool')
  ...
  CIQ.FibDialog.save = function() {
    ...
    if (CIQ.FibDialog.drawingToolbar) {
      ...
      if (CIQ.FibDialog.drawingToolbar.callback)
        CIQ.FibDialog.drawingToolbar.callback()
    }
    return fibonacci
  }
  ...
  /**
	 ...
	 * @example
		var config={
			...
			allowSymbolObject : false,                   // set to true to return a symbol object from the search instead of just a symbol string
  ...
  CIQ.LookupWidget.prototype.displayResults = function(results) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      if (result.exchange === null || typeof result.exchange == 'undefined')
        result.exchange = result.exchDisp
      if (result.exchange)
        exchangeSpan.appendChild(
          this.config.stx
            ? CIQ.translatableTextNode(this.config.stx, result.exchange)
            : result.exchange
        )
      if (this.config.allowSymbolObject)
        CIQ.ScrollManager.attach(li, select(this, result))
      else CIQ.ScrollManager.attach(li, select(this, result.symbol))
    }
    if (!this.iscroll) {
  ...
  CIQ.LookupWidget.prototype.display = function() {
    function pressFilter(that, div, filter) {
      return function() {
        ...
        that.config.textCallback(
          that,
          that.config.input.value,
          that.currentFilter
        )
      }
    }
    if (!this.div) {
      this.div = CIQ.newChild(
        this.config.input.parentNode,
        'DIV',
        'menuOutline stxLookupResults'
      )
      ...
      if (this.config.filters) {
        ...
        for (var i = 0; i < this.config.filters.length; i++) {
          ...
          div.appendChild(
            this.config.stx
              ? CIQ.translatableTextNode(this.config.stx, filter)
              : filter
          )
  ...
  CIQ.TimeZoneWidget.init = function() {
    if (typeof timezoneJS != 'undefined') {
      ...
      for (var i in timezoneJS.timezone.zones) {
        ...
        var s = i.split('/')
        var region = s[0]
        if (!CIQ.TimeZoneWidget.timezoneMenu[region])
          CIQ.TimeZoneWidget.timezoneMenu[region] = []
  ...
  CIQ.TimeZoneWidget.populateDialog = function(id) {
    ...
    if (!CIQ.TimeZoneWidget.iscroll) {
      CIQ.TimeZoneWidget.iscroll = CIQ.iscroll.newScroller(
  ...
  CIQ.ScrollManager.onContextMenu = function(e) {
    if (e.target.rightClickable) {
      // If node is right clickable then kill context menu, which will allow the mouseup event to trigger
      e.preventDefault()
      return false
    }
  ...
  document.addEventListener(
    'contextmenu',
    CIQ.ScrollManager.onContextMenu,
    false
  ) // To support right clicking
  ...
  CIQ.Watch.saveEditList = function() {
    ...
    CIQ.Watch.stxStorageManager.store(
      'stx-watchLists',
      JSON.stringify(CIQ.Watch.lists)
    )
  }
  ...
  CIQ.Watch.deleteCurrentList = function() {
    ...
    if (CIQ.Watch.currentList >= CIQ.Watch.lists.length)
      CIQ.Watch.currentList = CIQ.Watch.lists.length - 1
  ...
  CIQ.Watch.move = function(distance) {
    ...
    var list = CIQ.Watch.lists[CIQ.Watch.currentList]
    var symbols = list[CIQ.first(list)]
    if (CIQ.Watch.currentSymbol >= symbols.length)
      CIQ.Watch.currentSymbol = symbols.length - 1
    CIQ.Watch.enableSymbol(CIQ.Watch.currentSymbol)
    CIQ.Watch.symbolScroll.scrollToElement(
      '#stxWatch-inner li:nth-child(' + CIQ.Watch.currentSymbol + ')',
      250
    )
  ...
  CIQ.Watch.refreshDisplay = function() {
    ...
    function leftSymbol(ii) {
      return function(e) {
        CIQ.Watch.enableSymbol(ii)
      }
    }
    function rightSymbol(ii) {
      return function(e) {
        CIQ.Watch.rightClickSymbol(ii)
      }
    }
    function leftList(ii) {
      return function(e) {
        CIQ.Watch.enableList(ii)
      }
    }
  ...
  /**
	 ...
	 * @param  {CIQ.ChartEngine} stxStorageManager {@link CIQ.StorageManager} for getting and saving watch lists
	 * @param  {CIQ.ChartEngine} stxLookupWidget   {@link CIQ.LookupWidget} for changing the chart
	 * @memberOf CIQ.Watch
	 * @version ChartIQ Advanced Package
	 * @example
  ...
  CIQ.Watch.initialize = function(stxStorageManager, stxLookupWidget) {
    ...
    $$$('#stxWatchUp').onclick = (function(i) {
      return function() {
        CIQ.Watch.move(i)
      }
    })(-1)
    $$$('#stxWatchDown').onclick = (function(i) {
      return function() {
        CIQ.Watch.move(i)
      }
    })(1)
    $$$('#stxWatchPrev').onclick = (function(i) {
      return function() {
        CIQ.Watch.move(i)
      }
    })(-1)
    $$$('#stxWatchNext').onclick = (function(i) {
      return function() {
        CIQ.Watch.move(i)
      }
    })(1)
    ...
    CIQ.Watch.symbolScroll = CIQ.iscroll.newScroller(
      $$$('#stxWatch-symbols').parentNode,
      {
        tap: true,
        scrollbars: false,
        interactiveScrollbars: false,
    ...
    CIQ.Watch.listScroll = CIQ.iscroll.newScroller(
      $$$('#stxWatchLists').parentNode,
      {
        tap: true,
        scrollbars: false,
        interactiveScrollbars: false,
  ...
  CIQ.Views.saveView = function() {
    ...
    CIQ.Views.stxStorageManager.store(
      'stx-views',
      JSON.stringify(CIQ.Views.views)
    )
  }
  ...
  CIQ.Views.deleteView = function(i) {
    ...
    for (var j = 0; j < CIQ.Views.length; j++) {
      if (currentViewName == CIQ.first(CIQ.Views.views[j]))
        CIQ.Views.currentView = j
    }
    CIQ.Views.refreshDisplay()
  ...
  CIQ.Views.refreshDisplay = function() {
    ...
    function leftView(ii) {
      return function(e) {
        CIQ.Views.enableView(ii)
      }
    }
    function rightView(ii) {
      return function(e) {
        CIQ.Views.deleteView(ii)
      }
    }
    for (var i = 0; i < CIQ.Views.views.length; i++) {
      var view = CIQ.Views.views[i]
    ...
    scroller.style.width =
      panel.clientWidth - scroller.offsetLeft - rightMargin + 'px'
   ...
   * @param  {CIQ.ChartEngine} stx               The chart object
   * @param  {CIQ.ChartEngine} stxStorageManager CIQ.StorageManager
  ...
  CIQ.Views.initialize = function(stx, stxStorageManager, managePeriodicity) {
    ...
    CIQ.Views.scroll = CIQ.iscroll.newScroller(
      $$$('#stxSavedViews').parentNode,
      {
        tap: true,
        scrollbars: false,
        interactiveScrollbars: false,
  ...
  CIQ.SearchableWordList = function(list, maxResults, contains) {
    ...
    for (var r = 0; r < list.length; r++) {
      ...
      for (var j = 0; j < descs.length; j++) {
        ...
        var subIdx = '_'
        var subIdx2 = '_'
        if (word.charCodeAt(0) >= 33 && word.charCodeAt(0) <= 126)
          subIdx = word.charAt(0)
        if (!container.words[subIdx]) container.words[subIdx] = []
        if (word.length > 1) {
          if (word.charCodeAt(1) >= 33 && word.charCodeAt(1) <= 126)
            subIdx2 = word.charAt(1)
        } else {
          subIdx2 = ' '
        }
        if (!container.words[subIdx][subIdx2])
          container.words[subIdx][subIdx2] = []
        container.words[subIdx][subIdx2].push({
          index: record.index,
          word: word,
        })
      }
    ...
    this.lookup = function(input, category, cb) {
      ...
      if (input && container) {
        ...
        for (d = 0; d < container.records.length; d++) {
          ...
          if (keyword == '*') {
            exacts.push(
              CIQ.extend(container.records[entry.index], { weight: 0 })
            )
            ...
            if (i > -1) {
              ...
              ;(weight ? results : exacts).push(
                CIQ.extend(container.records[entry.index], { weight: weight })
              )
              foundBitMap[entry.index] = true
            }
          }
        ...
        var descResults = []
        if (myKey.charCodeAt(0) >= 33 && myKey.charCodeAt(0) <= 126)
          k1 = myKey.charAt(0)
        if (myKey.length > 1) {
          if (myKey.charCodeAt(1) >= 33 && myKey.charCodeAt(1) <= 126)
            k2 = myKey.charAt(1)
        } else {
          k2 = ' '
        }
        ...
        if (container.words[k1]) {
          for (var kk in container.words[k1]) {
            ...
            for (
              d = 0;
              container.words[k1][kk] && d < container.words[k1][kk].length;
              d++
            ) {
              ...
              if (
                category &&
                container.records[entry.index].category != category
              )
        ...
        for (var extraKeys = 1; extraKeys < keys.length; extraKeys++) {
          ...
          for (var res = descResults.length - 1; res >= 0; res--) {
            ...
            if (descResults[res].keywords)
              words = words.concat(descResults[res].keywords.split(' '))
            var match = false
        ...
        results = exacts
          .sort(sortWeight)
          .concat(results.sort(sortWeight), descResults.sort(sortDescWeight))
        results.length = Math.min(results.length, maxResults)
      }
  ...
  CIQ.Studies.studyDialog = function(stx, study, div, override) {
    ...
    else if (div.replaceID) delete div.replaceID
    ...
    for (i in sd.inputs) {
      ...
      newInput
        .querySelectorAll('.stx-heading')[0]
        .appendChild(CIQ.translatableTextNode(stx, i))
      ...
      var defaultValue =
        override &&
        override.inputs &&
        override.inputs[i] !== null &&
        typeof override.inputs[i] != 'undefined'
          ? override.inputs[i]
          : acceptedData
    ...
    for (i in sd.outputs) {
      ...
      if (override && override.outputs && override.outputs[i])
        value = override.outputs[i]
      if (value != 'auto') {
        colorClick.style.backgroundColor = value
        CIQ.unappendClassName(colorClick, 'stxColorDarkChart')
      } else {
        if (stx.defaultColor == '#FFFFFF')
          CIQ.appendClassName(colorClick, 'stxColorDarkChart')
      }
    ...
    if (parametersEL) {
      ...
      if (sd.parameters && sd.parameters.template && sd.parameters.init) {
        ...
        var template = document.querySelectorAll(
          '#' + sd.parameters.template
        )[0]
        if (template) {
          ...
          for (var f in sd.parameters.init) {
            ...
            if (
              override &&
              override.parameters &&
              typeof override.parameters[f] != 'undefined' &&
              override.parameters[f] !== null
            )
            ...
            if (el.tagName == 'INPUT') {
              ...
              if (pvalue == 'auto') {
                pvalue = ''
                if (stx.defaultColor == '#FFFFFF')
                  CIQ.appendClassName(el, 'stxColorDarkChart')
              } else {
  ...
  CIQ.Studies.parseDialog = function(div, stx) {
    ...
    for (i = 0; i < inputItems.length; i++) {
      if (inputItems[i].style.display != 'none') {
        ...
        var inputDOM = inputItems[i].querySelectorAll('.stx-data')[0]
          .childNodes[0]
    ...
    for (i = 0; i < outputItems.length; i++) {
      if (outputItems[i].style.display != 'none') {
        ...
        if (typeof field !== 'undefined') {
          var color = outputItems[i].querySelectorAll('.stx-color')[0].style
            .backgroundColor
          if (!color) color = 'auto'
          outputs[field] = color
        }
  ...
  CIQ.Marker.AboveCandle.placementFunction = function(params) {
    ...
    var stx = params.stx
    var useHighs =
      stx.chart.highLowBars || stx.highLowBars[stx.layout.chartType]
    ...
    for (var i = 0; i < params.arr.length; i++) {
      ...
      if (marker.params.xPositioner == 'bar') {
        ...
        node.style.left =
          Math.round(
            stx.pixelFromBar(marker.params.x, chart) - marker.clientWidth / 2
          ) +
          1 +
          'px'
      } else {
        ...
        if (!marker.tick && marker.tick !== 0) {
          ...
          if (marker.params.future && chart.scroll < chart.maxTicks) {
            // In future
        ...
        if (marker.tick < chart.dataSet.length)
          quote = chart.dataSet[marker.tick]
        ...
        marker.leftpx = Math.round(
          stx.pixelFromTick(marker.tick, chart) -
            chart.left -
      ...
      var height = marker.params.chartContainer
        ? stx.height
        : panel.yAxis.bottom
      ...
      if (useHighs) {
        bottom =
          Math.round(
            height - stx.pixelFromPrice(quote.High, panel, yAxis) + bottomAdjust
          ) + 'px'
      } else {
        bottom =
          Math.round(
            height -
              stx.pixelFromPrice(quote.Close, panel, yAxis) +
              bottomAdjust
          ) + 'px'
      }
