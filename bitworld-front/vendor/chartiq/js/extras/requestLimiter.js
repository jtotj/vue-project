...
;(function(definition) {
  ...
  if (typeof exports === 'object' && typeof module === 'object') {
    ...
    throw new Error(
      'Only CommonJS, RequireJS, and <script> tags supported for requestLimiter.js.'
    )
  }
})(
  /* endRemoveIf(umd) */ function(_exports) {
    ...
    CIQ.Extras.RequestLimiter.hitRequestLimit = function(url) {
      ...
      if (!CIQ.Extras.RequestLimiter.ajaxRequestLog)
        CIQ.Extras.RequestLimiter.ajaxRequestLog = {}
      ...
      for (var i = 0; i < log.length; i++) {
        text +=
          (i + 1).toString() + '. ' + log[i].time + '   ' + log[i].url + '\n'
      }
