...
;(function(definition) {
  ...
  if (typeof exports === 'object' && typeof module === 'object') {
    ...
    throw new Error(
      'Only CommonJS, RequireJS, and <script> tags supported for hlcbox.js.'
    )
  }
})(
  /* endRemoveIf(umd) */ function(_exports) {
    ...
    CIQ.ChartEngine.prototype.drawHLCBox = function(params) {
      ...
      for (var x = 0; x <= quotes.length; x++) {
        ...
        if (panel == chart.panel && chart.transformFunc && quote.transform)
        ...
        if (overlayScaling) {
          t1 =
            overlayScaling.bottom -
            (Top - overlayScaling.min) * overlayScaling.multiplier
          b1 =
            overlayScaling.bottom -
            (Bottom - overlayScaling.min) * overlayScaling.multiplier
          c1 =
            overlayScaling.bottom -
            (Close - overlayScaling.min) * overlayScaling.multiplier
        } else {
          t1 = yAxis.semiLog
            ? this.pixelFromTransformedValue(Top, panel, yAxis)
            : (yAxis.high - Top) * yAxis.multiplier + yAxis.top // inline version of pixelFromTransformedValue() for efficiency
          b1 = yAxis.semiLog
            ? this.pixelFromTransformedValue(Bottom, panel, yAxis)
            : (yAxis.high - Bottom) * yAxis.multiplier + yAxis.top // inline version of pixelFromTransformedValue() for efficiency
          c1 = yAxis.semiLog
            ? this.pixelFromTransformedValue(Close, panel, yAxis)
            : (yAxis.high - Close) * yAxis.multiplier + yAxis.top // inline version of pixelFromTransformedValue() for efficiency
        }
        yValueCache[x] = c1
        ...
        if (pxTop != pxBottom) {
          ...
          context.rect(
            ...
            pxTop - highlightOffset,
            Math.max(1, xend - xstart + 2 * highlightOffset),
            Math.max(1, pxBottom - pxTop + 2 * highlightOffset)
          )
          cache.push({
            left: xstart,
            right: xend,
