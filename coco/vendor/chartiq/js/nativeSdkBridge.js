...
function attachQuoteFeed(refreshInterval) {
  stxx.attachQuoteFeed(quoteFeedNativeBridge, {
...
function addDrawingListener() {
  stxx.addEventListener('drawing', function(drawingObject) {
    ...
    if (!isAndroid) {
      webkit.messageHandlers.drawingHandler.postMessage(
...
function addLayoutListener() {
  stxx.addEventListener('layout', function(layoutObject) {
    if (!isAndroid) {
      var seen = []
      webkit.messageHandlers.layoutHandler.postMessage(
        JSON.stringify(layoutObject.layout, function(key, val) {
          if (val !== null && typeof val == 'object') {
            if (seen.indexOf(val) >= 0) {
              return
            }
