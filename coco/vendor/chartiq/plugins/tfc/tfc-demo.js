...
CIQ.Account.Demo.ciqInheritsFrom(CIQ.Account)
...
CIQ.Account.Demo.prototype.fetchBalances = function(cb) {
  ...
  var self = this
  CIQ.postAjax(
    'https://jsfiddle.chartiq.com/sample_tfc_balances.js',
    null,
    function(status, myserverResponseData) {
      if (status != 200) {
        // something went wrong
        cb()
        return
      }
      if (typeof self.balances.cash == 'undefined')
        ...
        if (openOrder.tradeid && this.trades[symbol]) {
          for (var t = 0; t < this.trades[symbol].length; t++) {
            ...
            if (trade.id == openOrder.tradeid) {
              ...
              if (!trade.protect.stop && !trade.protect.limit)
                delete trade.protect
              break
            }
          }
        }
...
CIQ.Account.Demo.prototype.execute = function(order, price) {
  ...
  if (!position) {
    ...
    if (order.action == 'buy' || order.action == 'short') {
      position.basis = (
        (position.quantity * position.basis + quantity * price) /
  ...
  if (order.oto) {
    newTrade.protect = {}
    if (order.oto[0] && order.oto[0].stop)
      newTrade.protect.stop = order.oto[0].stop
    else if (order.oto[1] && order.oto[1].stop)
      newTrade.protect.stop = order.oto[1].stop
    if (order.oto[0] && order.oto[0].limit)
      newTrade.protect.limit = order.oto[0].limit
    else if (order.oto[1] && order.oto[1].limit)
      newTrade.protect.limit = order.oto[1].limit
  }
  ...
  if (newTrade.quantity && newTrade.protect) {
    ...
    if (newTrade.protect.limit)
      this.openOrders[order.symbol].push({
        id: uid + 'A',
        tradeid: newTrade.id,
        action: order.action == 'buy' ? 'sell' : 'cover',
        quantity: Math.abs(newTrade.quantity),
        limit: newTrade.protect.limit,
        tif: 'GTC',
        oco: newTrade.protect.stop ? uid + 'B' : null,
      })
    if (newTrade.protect.stop)
      this.openOrders[order.symbol].push({
        id: uid + 'B',
        tradeid: newTrade.id,
        action: order.action == 'buy' ? 'sell' : 'cover',
        quantity: Math.abs(newTrade.quantity),
        stop: newTrade.protect.stop,
        tif: 'GTC',
        oco: newTrade.protect.limit ? uid + 'A' : null,
      })
  }
  if (!this.openOrders[order.symbol].length)
...
CIQ.Account.Demo.prototype.closeTrade = function(tfc, trade, cb) {
  ...
  if (position.quantity == trade.quantity) {
    ...
    position.basis = (
      (position.quantity * position.basis - trade.quantity * trade.basis) /
