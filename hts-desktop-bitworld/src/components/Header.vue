  ...
  <header id="header-tag">
    ...
    <div :class="[c_long_locale ? 'long_wrapper' : '', 'wrapper']">
      <h1 class="logo" style="display:none;">
        <router-link :to="{ name: 'index' }"><img src="~assets/img/cocofx-logo-margin.png" alt=""></router-link>
      </h1>
      <nav class="gnb" style="height:60px;">
        <ul>
          <li>
          ...
          <li>
            <a href="https://cocofx.com/about_platform">{{ $t('header.manual') }}</a>
          </li>
          <li v-if="0">
      ...
      <div class="util" style="display:none;">
        ...
        <div v-if="is_logged && show_accounts" ref="type_sel" @click="show_options = !show_options" :class="['type_sel', { on: show_options }]">
          ...
          <ul v-if="c_trade_type_account.length">
            <li @click="selectAccount(account.account_srl)" v-for="account in c_trade_type_account">
              {{ fromPairs(constants.account.trade_types)[account.trade_type] }}[{{ fromPairs(constants.account.account_types)[account.account_type]}}]: 
