...
CIQ.TFC.prototype.updateValues=function(){
	...
	var abovePrice=this.quantityFromValue(this.elements.dragLineAbovePrice.innerHTML);
	var belowPrice=this.quantityFromValue(this.elements.dragLineBelowPrice.innerHTML);
...
CIQ.TFC.prototype.deriveOpenOrderMarkers=function(){
	...
	if( !this.account.config.showOpenOrdersWhenTFCClosed && CIQ.hasClassName($$$(".stx-trade-panel"), "closed")) return;
...
CIQ.TFC.prototype.updateData=function(){
	...
	function update(){
		...
		$$$(".tfc-liquidity").innerHTML=CIQ.money(self.account.balances.liquidity, null, CIQ.convertCurrencyCode(self.account.currency));
		$$$(".tfc-unsettled-cash").innerHTML=CIQ.money(self.account.balances.unsettledCash, null, CIQ.convertCurrencyCode(self.account.currency));
		$$$(".tfc-cash").innerHTML=CIQ.money(self.account.balances.cash, null, CIQ.convertCurrencyCode(self.account.currency));
		if(self.account.balances.profitLoss){
			$$$(".tfc-profitloss").parentNode.style.display="";
			$$$(".tfc-profitloss").innerHTML=CIQ.money(self.account.balances.profitLoss, null, CIQ.convertCurrencyCode(self.account.currency));
		}else{
			$$$(".tfc-profitloss").parentNode.style.display="none";
		}
		$$$(".tfc-buying-power").innerHTML=CIQ.money(self.account.balances.buyingPower, null, CIQ.convertCurrencyCode(self.account.currency));
						...
						if(viewType=="maintenance"){
							...
							if(trade.protect){
								if(trade.protect.limit) {
									var tp=CIQ.newChild(td, "SPAN", null, "\u00a0@TP\u00a0"+trade.protect.limit);
									tp.style.color="green";
									protection=true;
								}
								if(trade.protect.stop) {
									var sl=CIQ.newChild(td, "SPAN", null, "\u00a0@SL\u00a0"+trade.protect.stop);
									sl.style.color="red";
									protection=true;
								}
							}
...
/*		$$$(".stx-trade-positions").style.display="none";
	...
	if(this.positionIScroll===null){
		this.positionIScroll=CIQ.iscroll.newScroller(positionWrapper, {tap:true, scrollbars: true, interactiveScrollbars: true});
	}else{
		this.positionIScroll.refresh();
	}
	if(this.openOrderIScroll===null){
		this.openOrderIScroll=CIQ.iscroll.newScroller(openOrderWrapper, {tap:true, scrollbars: true, interactiveScrollbars: true});
	}else{
	...
	this.account.confirmOrder(this, order, function(confirmation){
		...
		if(confirmation && confirmation.errors){
			...
			for(i=0;i<confirmation.errors.length;i++){
				div=CIQ.newChild($$$("#tfcConfirmOrder .tfc-errors"), "div", null, confirmation.errors[i]);
			}
		}else{
		...
		if(confirmation && confirmation.warnings){
			for(i=0;i<confirmation.warnings.length;i++){
				div=CIQ.newChild($$$("#tfcConfirmOrder .tfc-warnings"), "div", null, confirmation.warnings[i]);
			}
		}else{
...
CIQ.TFC.prototype.createOCOFromGUI=function(){
	...
	if(this.activeTrade=="strangle"){
		...
		order.action="sell";
		order.limit=this.quantityFromValue(this.elements.dragLineAbovePrice.innerHTML);
		order2.action="buy";
		order2.limit=this.quantityFromValue(this.elements.dragLineBelowPrice.innerHTML);
		...
		order2.quantity=this.quantityFromValue(this.elements.ocoBelowShares.innerHTML);
		order.action="buy";
		order.stop=this.quantityFromValue(this.elements.dragLineAbovePrice.innerHTML);
		order2.action="sell";
		order2.stop=this.quantityFromValue(this.elements.dragLineBelowPrice.innerHTML);
		order.tif=this.elements.ocoTIF.value;
		order2.tif=this.elements.ocoTIF.value;
...
CIQ.TFC.prototype.confirmOrder=function(order){
	...
	this.account.confirmOrder(this, order, function(confirmation){
		...
		function closure(self, order, submitButton){
			return function(){
				if(!order.quantity && order.quantity!==0) order.quantity=$$("vsp_quantity").value;
...
CIQ.TFC.prototype.setResizeCallback=function(resizeCallback){
	var self=this;
    CIQ.safeClickTouch($$$(".stx-trade-nav .stx-trade-ticket-toggle"), function(stx){
      return function(){
        stx.preAdjustScroll();
    ...
    CIQ.safeClickTouch($$$(".stx-trade-info .stx-trade-ticket-toggle"), function(stx){
      return function(){
        stx.preAdjustScroll();
...
CIQ.TFC.prototype.construct=function(config){
	...
	CIQ.safeDrag(this.dom.ocoOrder,
		function(self, node){ return function(e){ self.startDrag(e, node);};}(this, this.dom.dragLineCenter),
		function(self){ return function(e){ self.dragCenterLine(e);};}(this),
		function(self){ return function(e){ self.endDrag(e, self.dom.dragLineCenter);};}(this)
	);
