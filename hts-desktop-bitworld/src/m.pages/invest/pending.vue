	...
	<div class="pending" v-if="is_loaded">
        <span data-role="select">
            <select v-model="selected_account_srl" @change="loadTradeList()">
                <option :value="0">{{ accounts.length ? $t('view_total_account_not_execution') : $t('no_inquiry_possibility_account') }}</option>
        ...
        <div class="none_list" v-if="!c_trade_list.length">{{ $t('not_execution_order_exist') }}</div>
		<div class="trade" v-for="trade_info in c_trade_list" :data-status="trade_info.status">
			...
			<div class="my_box">
				<div class="left_col">
					<span class="name">{{ $t('entry_reservation') }}</span>
    ...
    methods: {
        ...
        loadTradeList(callback) {
            ...
            vm.$request(function(error, { data }) {
                ...
                (callback || function() {})()
            }, 'get', '/api/trade', { status: ['PENDING'].join(), account_srl: this.selected_account_srl || null, trade_type: this.trade_method, list_count: 50 });
        },
