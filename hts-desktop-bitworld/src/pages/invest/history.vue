<template>
	<div class="history" v-if="is_loaded">
        <span data-role="select">
            <select v-model="selected_account_srl" @change="loadTradeList()">
                <option :value="0">{{ accounts.length ? '전계좌 거래내역 조회' : '조회 가능 계좌가 없습니다.' }}</option>
        ...
        <div class="none_list" v-if="!c_trade_list.length">종결된 주문이 존재하지 않습니다.</div>
		<div class="trade" v-for="trade_info in c_trade_list" :data-status="trade_info.status">
			...
			<div class="my_box">
                ...
                <div class="info" v-if="trade_info.status != 'CANCELD'">
                    <dl>
                        <dt>손익금</dt>
                        <dd>{{ php.number_format(trade_info.gain_profit) }}</dd>
                    </dl>
                    <dl>
                        <dt>수수료</dt>
    ...
    methods: {
        ...
        loadTradeList(callback) {
            ...
            vm.$request(function(error, { data }) {
                ...
                (callback || function() {})()
            }, 'get', '/api/trade', { status: ['CLEARED', 'CANCELD'].join(), account_srl: this.selected_account_srl || null, list_count: 50 });
        },
