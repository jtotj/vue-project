            ...
            <table>
                ...
                <thead>
                    <tr>
                        ...
                        <th style="color:#373737; background:#ddd;" scope="col">{{ $t('hits') }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="document in document_list">
                        <td class="a_l"><router-link :to="{ name: 'noticeView', params: document }">{{ document.title }}</router-link></td>
                        <td>{{ func.zdate(document.regdate, 'Y-m-d') }}</td>
                        <td>{{ php.number_format(document.readed_count) }}</td>
                    </tr>
                    <tr v-if="!document_list.length" >
                        <td colspan="3"><br /><br /><br />{{ $t('csnav.no_notice') }}<br /><br /><br /><br /></td>
                    </tr>
                </tbody>
            </table>
        </div>
